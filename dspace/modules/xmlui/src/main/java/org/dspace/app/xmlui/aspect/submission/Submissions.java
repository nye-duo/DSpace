...
//        this.addSubmissionsInWorkflowDiv(div);
    ...
    private void addUnfinishedSubmissions(Division division) throws SQLException, WingException
    {
        ...
        if (unfinishedItems.size() > 0)
        {
            ...
            for (WorkspaceItem workspaceItem : unfinishedItems)
            {
                ...
                String cfg = configurationService.getProperty("cristin.unitcode.field");
                String unitcodes = "";
                if (cfg != null)
                {
                    List<MetadataValue> unitcodeDC = itemService.getMetadataByMetadataString(workspaceItem.getItem(), cfg);
                    // DCValue[] unitcodeDC = workspaceItem.getItem().getMetadata(cfg);
                    if (unitcodeDC.size() > 0)
                    {
                        for (MetadataValue uc : unitcodeDC)
                        {
                            if (!"".equals(unitcodes)) { unitcodes += ", "; }
                            unitcodes += uc.getValue();
                        }
                    }
                }
                ...
                // get the authors
                String authors = "";
                List<MetadataValue> authorDC = itemService.getMetadataByMetadataString(workspaceItem.getItem(), "dc.creator.author");
            ...
            throws SQLException,WingException
    {
        ...
        while(i.hasNext())
        {
            ...
            List<MetadataValue> titles = itemService.getMetadata(published, "dc", "title", null, Item.ANY);
            String collectionName = published.getOwningCollection().getName();
            List<MetadataValue> ingestDate = itemService.getMetadata(published, "dc", "date", "accessioned", Item.ANY);
            ...
            if (cfg != null)
            {
                List<MetadataValue> unitcodeDC = itemService.getMetadataByMetadataString(published, cfg);
            ...
            // get the authors
            String authors = "";
            List<MetadataValue> authorDC = itemService.getMetadataByMetadataString(published, "dc.creator.author");
