...
public class DuoState
{
            ...
            throws DuoException
    {
        ...
        if (val != null)
        {
            StateDetails sd = this.parseSource(val.getValue());
            this.details = sd;
        }
        else
        {
     ...
     */
    public void sychroniseItemState(Context context, boolean itemUpdate)
            throws DuoException, SQLException, AuthorizeException
    {
        ...
        this.recordStateOnItem(context);
        if (itemUpdate)
        {
            ItemService itemService = ContentServiceFactory.getInstance().getItemService();
     ...
     * @throws DuoException
     */
    public void recordStateOnItem(Context context)
            throws DuoException
    {
        try
        {
            MetadataFieldRepresentation dcv = this.metadataManager.makeDCValue(this.field, null);
            ItemService itemService = ContentServiceFactory.getInstance().getItemService();
            ...
            itemService.addMetadata(context, item, dcv.schema, dcv.element, dcv.qualifier, null, stateVal);
        }
    ...
    private String getRawEmbargoDate(Item item)
    {
        ...
        return embargoes.get(0).getValue();
    }
}
