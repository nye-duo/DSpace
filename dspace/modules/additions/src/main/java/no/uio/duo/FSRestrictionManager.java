...
public class FSRestrictionManager
{
    ...
    private boolean isPass(Item item)
    {
        ...
        {
            if (DuoConstants.FS_PASS.equals(dcv.getValue().trim()))
            {
                return true;
            }
        }
    ...
    private boolean isRestricted(Item item)
    {
        ...
        for (MetadataValue dcv : dcvs)
        {
            if (DuoConstants.FS_RESTRICTED.equals(dcv.getValue().trim()))
            {
                return true;
            }
        }
     ...
     * @throws IOException
     */
    private void original2Admin(Context context, Item item)
            throws SQLException, AuthorizeException, IOException
    {
        ...
        List<Bundle> admins = itemService.getBundles(item, DuoConstants.ADMIN_BUNDLE);
        if (admins.size() > 0)
        {
            admin = admins.get(0);
        }
        ...
        {
            admin = bundleService.create(context, item, DuoConstants.ADMIN_BUNDLE);
        }
        ...
        while (bsi.hasNext())
        {
            ...
            bundleService.addBitstream(context, admin, bs);
     ...
     * @throws IOException
     */
    private void withdraw(Context context, Item item)
            throws SQLException, AuthorizeException, IOException
    {
