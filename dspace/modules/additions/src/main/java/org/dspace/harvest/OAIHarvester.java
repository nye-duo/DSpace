...
public class OAIHarvester
{
			...
			throws SQLException, IOException, AuthorizeException
	{
		...
		if (ePerson == null)
		{
			String adminEperson = configurationService.getProperty("oai.admin.eperson");
			ePerson = epersonService.findByEmail(ourContext, adminEperson);
			if (ePerson == null)
			{
				ePerson = epersonService.findByNetid(ourContext, adminEperson);
			}
		}
		if (ePerson == null)
		{
    ...
    protected void processRecord(Element record, String OREPrefix, final long currentRecord, long totalListSize) throws SQLException, AuthorizeException, IOException, CrosswalkException, HarvestingException, ParserConfigurationException, SAXException, TransformerException
    {
    	...
    	if (item != null) // found an item so we modify
    	{
			...
			if (mdAuthority != null)
			{
				mdr = (MetadataRemover) pluginService.getNamedPlugin(MetadataRemover.class, mdAuthority);
			}
			if (mdr != null)
			{
    		...
    		if (harvestRow.getHarvestType() == 3) {
				...
				if (updateBitstreams)
				{
					...
					{
						bvs = (BundleVersioningStrategy) pluginService.getNamedPlugin(BundleVersioningStrategy.class, bundleVersioning);
					}
					if (bvs != null)
					{
    		...
    		// exists in a harvest collection but does not have an OAI_id)
    	{
			...
			// trigger the post-create behaviour for this ingestion workflow
			item = ingestionWorkflow.postCreate(ourContext, wi, handle);

			/*
			 * Don't automatically ingest the item, this is for the postCreate handler above to decide
    		try {
